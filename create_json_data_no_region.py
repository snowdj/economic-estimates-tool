...
df = pd.read_excel('GVA_sector_tables.xlsx', sheet_name='1.1 - GVA current (£bn)', skiprows=[0,1,2,3,4])
df = df.iloc[pd.np.r_[0:10, 11], :]
df = df.drop(df.columns[-3:], axis=1)
df = df.set_index('Sector')
df = df.rename(index={'Civil Society (Non-market charities)': 'Civil Society'})
df = df.rename(index={'All DCMS sectors': 'All DCMS Sectors'})
df = df.drop('All DCMS sectors (exc Tourism)')
df.columns.name = 'Year'
df = df.stack()
df.name = 'GVA'
gva = df.copy()

df = pd.read_excel('Tables_1-19-DCMS_Sectors_Economic_Estimates_Employment_2017_tables.xlsx', sheet_name="1 - Employment (000's)", skiprows=[0,1,2,3,4])
df = df.iloc[pd.np.r_[0:9, 10], :]
df = df.drop(df.columns[-3:], axis=1)
df = df.set_index('Sector')
df = df.rename(index={'All UK Sectors': 'UK'})
df = df.rename(index={'All DCMS sectors': 'All DCMS Sectors'})
df.columns.name = 'Year'
df = df.stack()
df.name = 'Employment'
...
df = pd.read_excel('Tables_1-6_Exports_and_imports_of_services_by_sector.xlsx', sheet_name="1 Exports 2010-16 (£m)", skiprows=[0,1,2,3,4, 5, 6])
df = df.iloc[pd.np.r_[0:8], :]
df = df.drop(df.columns[-3:], axis=1)
df = df.rename(columns={'Unnamed: 0': 'Sector'})
df = df.set_index('Sector')
df = df.rename(index={'Total value of DCMS exports of services (all sectors excluding Tourism and Civil Society)4': 'All DCMS Sectors'})
df = df.rename(index={'Total value of UK exports of services (ONS Balance of Payments, Pink Book)5': 'UK'})
df.columns.name = 'Year'
df = df.stack()
df.name = 'Exports'
exp = df.copy()

df = pd.read_excel('Tables_1-6_Exports_and_imports_of_services_by_sector.xlsx', sheet_name="4 Imports 2010-16 (£m)", skiprows=[0,1,2,3,4, 5, 6])
df = df.iloc[pd.np.r_[0:8], :]
df = df.drop(df.columns[-3:], axis=1)
df = df.rename(columns={'Unnamed: 0': 'Sector'})
df = df.set_index('Sector')
df = df.rename(index={'Total value of DCMS imports of services (all sectors excluding Tourism and Civil Society)4': 'All DCMS Sectors'})
df = df.rename(index={'Total value of UK imports of services (ONS Balance of Payments, Pink Book)5': 'UK'})
df.columns.name = 'Year'
df = df.stack()
df.name = 'Imports'
imp = df.copy()

df = pd.concat([gva, employment, exp, imp], axis=1)
df.to_csv('static/data/ee_agg_no_region.csv')
